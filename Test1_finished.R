...
library(GGally)
df <- read_csv("cleaned_covid_data.csv")
Astates1<- grepl("A", df$Province_State, ignore.case = FALSE, perl = FALSE,
...
ggplot(data=Astates, aes(x=Last_Update, y=Active)) +
  geom_point()+
...
States <- df[order(df$Province_State),]
States1 <- subset(States, select = -c(Last_Update,Deaths,Recovered,Active,Confirmed) )
chicken<- States1[order(-States$Case_Fatality_Ratio),]
salsa <- chicken$Province_State
state_max_fatality_rate<- chicken[!duplicated(salsa),]
...
beef<- state_max_fatality_rate[order(state_max_fatality_rate$Province_State),]
candy <- beef[-c(9),] 
final <- cbind(candy,state.region)
 final %>% 
   ggplot(aes(x=state.region, y=Case_Fatality_Ratio))+
   geom_boxplot()+
   labs(x="Region", y="Fatalities", title="Regional Fatalities")
